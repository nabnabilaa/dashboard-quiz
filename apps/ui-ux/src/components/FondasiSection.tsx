...
export const FondasiSection = () => {
    const lang = useLang();
    const t = ui[lang].material;
    const items = modules[lang];

    const [idx, setIdx] = useState(0);
    const mod = items[Math.min(idx, items.length - 1)];
    ...
    return (
        <div className="flex min-h-[500px] md:min-h-[600px] animate-[slide-up_0.5s_cubic-bezier(0.16,1,0.3,1)_forwards] flex-col overflow-hidden rounded-[20px] border border-white bg-white shadow-md">
            ...
            <div className="flex items-center justify-between border-t border-slate-200 bg-white px-5 py-4 md:px-10 md:py-5">
                ...
                >
                    <ChevronLeft size={18} /> <span className="hidden sm:inline">{t.previous}</span>
                </button>
                ...
                <button
                    className={`flex items-center gap-2 rounded-xl bg-indigo-600 px-3 py-2 md:px-5 md:py-2.5 text-xs md:text-sm font-semibold text-white transition-all hover:bg-indigo-700 ${idx === items.length - 1 ? 'cursor-not-allowed opacity-50' : 'cursor-pointer'
                        }`}
                    disabled={idx === items.length - 1}
                    onClick={() => setIdx((i) => i + 1)}
                >
                    <span className="hidden sm:inline">{t.next}</span> <ArrowRight size={18} />
                </button>
            </div>
        </div>
    );
