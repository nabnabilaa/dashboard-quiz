    ...
    return (
        <div className="flex flex-col items-center gap-6 md:flex-row md:gap-10">
            <div className="text-center">
                ...
                >
                    {state === 'default' && v.buttonState.idle}
                    {state === 'hover' && v.buttonState.hover}
                    {state === 'pressed' && v.buttonState.pressed}
                </button>
                <p className="mt-4 text-[13px] text-slate-500">{v.buttonState.hint}</p>
            </div>
            <div className="flex flex-col gap-3 rounded-xl border border-slate-200 bg-white p-4">
                <div className="flex items-center gap-2.5 text-xs text-slate-600">
                    <div className="h-3 w-3 rounded-[3px] bg-indigo-600"></div> {v.buttonState.normalLabel}
                </div>
                <div className="flex items-center gap-2.5 text-xs text-slate-600">
                    <div className="h-3 w-3 rounded-[3px] bg-indigo-700 shadow shadow-indigo-700"></div> {v.buttonState.hoverLabel}
                </div>
                <div className="flex items-center gap-2.5 text-xs text-slate-600">
                    <div className="h-3 w-3 rounded-[3px] bg-indigo-900"></div> {v.buttonState.pressedLabel}
                </div>
            </div>
        </div>
    );
