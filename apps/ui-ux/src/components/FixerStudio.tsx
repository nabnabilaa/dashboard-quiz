...
import { fixerVisuals } from '@belajar/i18n/visuals';
...
const RenderFixerVisual = ({ type, fixed }: RenderFixerVisualProps) => {
    ...
    if (type === 'spacing') return <div className={`${containerStyle} ${fixed ? 'p-5' : 'p-0'}`}><div className="w-full h-[50px] bg-blue-500"></div><div className={fixed ? 'p-0' : 'p-[2px]'}>{v.spacingText}</div></div>;
    if (type === 'cta_color') return <div className={containerStyle}><button className={`px-5 py-2.5 rounded-lg border-none ${fixed ? 'bg-indigo-600 text-white' : 'bg-slate-200 text-slate-400'}`}>{v.buyButton}</button></div>;
    ...
    if (type === 'contrast_fix') return <div className={containerStyle}><span className={`text-[20px] font-bold ${fixed ? 'text-slate-800' : 'text-slate-100'}`}>{v.helloWorld}</span></div>;
    if (type === 'alignment_fix') return <div className={containerStyle}><p className={`text-xs ${fixed ? 'text-left' : 'text-center'}`}>{v.alignmentParagraph}</p></div>;
    if (type === 'input_label_fix') return <div className={containerStyle}>{fixed && <label className="self-start text-[10px]">{v.emailLabel}</label>}<input placeholder={fixed ? "" : v.emailPlaceholder} className="w-full p-1.5 border border-slate-300 rounded" /></div>;
    if (type === 'touch_target_fix') return <div className={containerStyle}><button className={`rounded-lg border-none bg-indigo-600 text-white ${fixed ? 'px-[30px] py-[15px] text-base' : 'p-[5px] text-[10px]'}`}>{v.tapMe}</button></div>;
    if (type === 'modal_close_fix') return <div className={`${containerStyle} relative bg-slate-800`}><div className="bg-white w-[150px] h-[100px] rounded-lg relative">{fixed && <div className="absolute top-[5px] right-[5px] cursor-pointer"><X size={14} /></div>}<div className="text-center mt-[30px]">{v.ad}</div></div></div>;
    if (type === 'hierarchy_fix') return <div className={containerStyle}><div className={`${fixed ? 'text-2xl font-bold' : 'text-sm font-normal'}`}>{v.mainHeading}</div><div className="text-sm">{v.subHeading}</div></div>;
    if (type === 'error_msg_fix') return <div className={containerStyle}><input className="border border-red-500 p-1.5 rounded" />{fixed && <div className="text-red-500 text-[10px] mt-[5px]">{v.invalidEmail}</div>}</div>;
    if (type === 'empty_state_fix') return <div className={containerStyle}>{fixed ? <div className="text-center"><Search className="mx-auto" /><div className="text-[10px]">{v.empty}</div><button className="text-[10px] mt-[5px] px-[5px] border border-slate-300 rounded">{v.search}</button></div> : <div className="text-xl text-slate-300">404</div>}</div>;
    ...
    if (type === 'menu_simplify_fix') return <div className={containerStyle}>{fixed ? <Menu /> : <div className="text-[8px]">{v.crowdedMenu}</div>}</div>;
    if (type === 'dark_mode_fix') return <div className={`${containerStyle.replace('bg-white', '')} text-white ${fixed ? 'bg-[#121212]' : 'bg-black'}`}>{fixed ? v.darkGray : v.pureBlack}</div>;
...
export const FixerStudio = () => {
    const lang = useLang();
    const t = ui[lang].fixer;
    const levels = fixers[lang];
    ...
    const task = !isCompleted ? levels[Math.min(level, levels.length - 1)] : levels[0];
    ...
    if (isCompleted) {
        return (
            <div className="flex min-h-[500px] flex-col items-center justify-center rounded-[20px] bg-slate-100 p-10 text-center shadow-inner">
                ...
                <h2 className="mb-4 text-3xl font-extrabold text-slate-900">{t.doneTitle}</h2>
    ...
    return (
        <div className="grid min-h-[500px] grid-cols-1 gap-[30px] md:grid-cols-[1fr_320px]">
            ...
            <div className="flex flex-col justify-center rounded-[20px] border border-white bg-white p-6 shadow-md">
                <h3 className="mb-2 text-sm font-bold uppercase tracking-wider text-slate-400">{t.level(level + 1)}</h3>
                <h2 className="mb-4 text-xl font-extrabold text-slate-900">{task.title}</h2>
                <p className="mb-6 text-slate-600">{isFixed ? t.fixed : task.problem}</p>
