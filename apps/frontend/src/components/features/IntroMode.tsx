...
export function IntroMode({ onFinish }: IntroModeProps) {
    ...
    return (
        <div className="flex flex-col items-center justify-center h-full bg-[#F8FAFC] p-4 md:p-6 relative overflow-auto">
            <div className="max-w-5xl w-full flex flex-col lg:flex-row gap-6 lg:gap-8 items-stretch justify-center min-h-0 lg:h-full">
              ...
              inline-block
            `}>
                            {t.slideCounter(slideIdx + 1, slides.length)}
                        </span>
                    </div>
              ...
              `}
                        >
                            {isLastSlide ? t.startQuiz : t.continue}
                            <ArrowRight size={18} />
                        </button>
                    </div>
                </div>
