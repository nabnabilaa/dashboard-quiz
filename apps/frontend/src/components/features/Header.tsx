...
import { Book, GraduationCap, PenTool, Palette, Menu, X } from 'lucide-react';
...
export function Header({ phase, setPhase }: HeaderProps) {
    ...
    const navItems: { target: Phase; icon: React.ElementType; label: string; color: string }[] = [
        { target: 'intro', icon: Book, label: t.intro, color: 'purple' },
        { target: 'quiz', icon: GraduationCap, label: t.quiz, color: 'pink' },
        { target: 'workshop', icon: PenTool, label: t.workshop, color: 'orange' },
        { target: 'builder', icon: Palette, label: t.builder, color: 'indigo' },
    ];
        ...
        `}
            >
                <div className="p-4 border-b border-slate-200 flex justify-between items-center">
                    <span className="text-slate-800 font-bold">{t.menu}</span>
                    <button
                        onClick={() => setMobileMenuOpen(false)}
