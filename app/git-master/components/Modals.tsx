        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4 overflow-y-auto backdrop-blur-sm animate-fade-in">
            ...
            {activeModal === 'intro' && (
                <div className="bg-white border border-slate-200 rounded-2xl w-full max-w-4xl shadow-2xl my-8 overflow-hidden">
                    <div className="bg-indigo-600 p-6 text-center relative overflow-hidden">
                        ...
                        <p className="text-indigo-200 text-sm relative z-10 mt-1">Read this first so you don't get lost!</p>
                    </div>
                    <div className="p-6 md:p-8 overflow-y-auto max-h-[70vh]">
                        <div className="grid md:grid-cols-2 gap-6">
                            <div className="space-y-4">
                                <h3 className="text-indigo-600 font-bold text-xs uppercase tracking-widest border-b border-indigo-200 pb-2">Basic Flow</h3>
                                <div className="flex gap-4 items-start group">
                                    ...
                                    <div><h4 className="font-bold text-slate-800 text-sm">Working Directory</h4><p className="text-xs text-slate-500 mt-1">Where you code. Start with <code className="bg-slate-100 px-1 rounded text-red-500">git init</code>.</p></div>
                                </div>
                                <div className="flex gap-4 items-start group">
                                    ...
                                    <div><h4 className="font-bold text-slate-800 text-sm">Staging Area</h4><p className="text-xs text-slate-500 mt-1">Shopping cart before saving. <code className="bg-slate-100 px-1 rounded text-yellow-600">git add</code></p></div>
                                </div>
                                <div className="flex gap-4 items-start group">
                                    ...
                                    <div><h4 className="font-bold text-slate-800 text-sm">Repository (Local)</h4><p className="text-xs text-slate-500 mt-1">Save permanent history. <code className="bg-slate-100 px-1 rounded text-green-600">git commit</code></p></div>
                                </div>
                            </div>
                            <div className="space-y-4">
                        ...
                        <div className="flex gap-4 items-start group">
                            ...
                            <div><h4 className="font-bold text-slate-800 text-sm">Branching</h4><p className="text-xs text-slate-500 mt-1">Create new paths for safety. <code className="bg-slate-100 px-1 rounded text-pink-500">git branch</code></p></div>
                        </div>
                        <div className="flex gap-4 items-start group">
                            ...
                            <div><h4 className="font-bold text-slate-800 text-sm">Merging</h4><p className="text-xs text-slate-500 mt-1">Merge code into main. <code className="bg-slate-100 px-1 rounded text-purple-500">git merge</code></p></div>
                        </div>
                        <div className="flex gap-4 items-start group">
                            ...
                            <div><h4 className="font-bold text-slate-800 text-sm">Push & Pull</h4><p className="text-xs text-slate-500 mt-1">Send & Fetch from GitHub. <code className="bg-slate-100 px-1 rounded text-blue-500">git push/pull</code></p></div>
                        </div>
                            </div>
                        </div>
                    </div>
            ...
            {activeModal === 'mode' && (
                <div className="bg-white border border-slate-200 rounded-2xl w-full max-w-4xl shadow-2xl my-8 overflow-hidden">
                    <div className="bg-indigo-600 p-8 text-center relative overflow-hidden">
                        <div className="absolute inset-0 bg-white opacity-10 pattern-dots"></div>
                        <h2 className="text-3xl font-bold text-white relative z-10 tracking-tight">CHOOSE LEARNING MODE</h2>
                        <p className="text-indigo-200 text-sm relative z-10 mt-2">Tailor it to your learning style</p>
                    </div>
                    ...
                    <div className="p-6 md:p-10 grid md:grid-cols-2 gap-6">
                        <div className="bg-blue-50 p-6 rounded-2xl border border-blue-200 hover:border-blue-400 hover:shadow-lg transition-all cursor-pointer group flex flex-col items-center text-center" onClick={() => onSelectMode('quiz')}>
                            ...
                            <p className="text-sm text-slate-500 mb-6">Answer multiple choice questions. Perfect for beginners to understand concepts.</p>
                            <button className="bg-blue-600 hover:bg-blue-500 text-white font-bold py-2 px-6 rounded-full w-full">Start Quiz</button>
                        </div>
                        ...
                        <div className="bg-green-50 p-6 rounded-2xl border border-green-200 hover:border-green-400 hover:shadow-lg transition-all cursor-pointer group flex flex-col items-center text-center" onClick={() => onSelectMode('training')}>
                            ...
                            <h3 className="text-xl font-bold text-slate-800 mb-2">Terminal Mode</h3>
                            <p className="text-sm text-slate-500 mb-6">Challenge of typing commands manually. Feel the thrill of being a programmer.</p>
                            <button className="bg-green-600 hover:bg-green-500 text-white font-bold py-2 px-6 rounded-full w-full">Start Terminal</button>
                        </div>
                    </div>

                    <div className="bg-slate-50 p-4 text-center border-t border-slate-200">
                        <button onClick={onBackToIntro} className="text-slate-400 hover:text-slate-700 text-sm underline">
                            <i className="fas fa-arrow-left mr-1"></i> Back to Pocket Dictionary
                        </button>
                    </div>
                </div>
            )}
            ...
            {activeModal === 'end' && (
                <div className="bg-white border-2 border-amber-400 rounded-2xl max-w-md w-full p-8 text-center relative overflow-hidden shadow-2xl">
                    ...
                    <h2 className="text-2xl font-bold text-slate-800 mb-2">MISSION ACCOMPLISHED!</h2>
                    <p className="text-slate-500 text-sm mb-6">
                        {mode === 'quiz'
                            ? "You've mastered Git theory. Try Terminal Mode for more challenges!"
                            : "Great! You're ready to use Git in a real terminal."}
                    </p>
                    <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 mb-6">
                        <span className="text-slate-400 text-xs uppercase tracking-widest">Final Score</span>
